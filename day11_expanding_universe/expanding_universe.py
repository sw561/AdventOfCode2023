...
def get_distance(left, right, expanded, n):
    if left > right:
        left, right = right, left
    extra = n * sum(1 for e in expanded if left < e < right)
    return right - left + extra

def solve_part1(data, n=1):
    ...
    for star1, star2 in combinations(stars, 2):
        ...
        y_distance = get_distance(star1[0], star2[0], expanding_rows, n)
        x_distance = get_distance(star1[1], star2[1], expanding_cols, n)
...
def main():
    ...
    return solve_part1(data), solve_part1(data, 1_000_000 - 1)
